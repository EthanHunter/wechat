...
defined('ACCESS_TOKEN') || define('ACCESS_TOKEN', 'access_token_');
...
class TokenGetWithDb extends TokenGetBase implements AccessToken
{
    /**
     * 微信api根路径
     * @var string
    ...
    public function getAccessToken($appId, $appSecret)
    {
        ...
        // access_token 存在则直接赋值并返回
        $access_token_set = M('access_token')
            ->where(array('app_id' => $appId, 'app_secret' => $appSecret))
            ->find();
        if ($access_token_set) {
            if ($access_token_set['expires_in'] > time()) {
                //未超时，直接返回access_token
                return array('access_token' => $access_token_set['access_token']);
            }
        }
        ...
        if (is_array($token)) {
            if (isset($token['errcode'])) {
                ...
                if ($access_token_set) {
                    ...
                    $data['expires_in'] = time() + intval($token['expires_in']);
...
class TokenGetWithRedis extends TokenGetBase implements AccessToken
{
    /**
     * 微信api根路径
     * @var string
    ...
    public function getAccessToken($appId, $appSecret)
    {
        ...
        // access_token 存在则直接赋值并返回
        $access_token = $redis->get(ACCESS_TOKEN . $appId);
        if ($access_token) {
        ...
        if (is_array($token)) {
            if (isset($token['errcode'])) {
                ...
                //将access_token以有效时间为7000秒存储至redis
                $redis->setex(ACCESS_TOKEN . $appId, 7000, $token['access_token']);
                return $token;
            }
        } else {
            throw new \Exception('获取微信access_token失败！');
...
class TokenGetWithFile extends TokenGetBase implements AccessToken
{
    /**
     * 微信api根路径
     * @var string
    ...
    public function getAccessToken($appId, $appSecret)
    {
        $accessTokenFilePath = 'Runtime/'.ACCESS_TOKEN.$appId.'.php';
        // TODO: Implement getAccessToken() method.
        $data = json_decode($this->get_php_file($accessTokenFilePath));
        ...
        if (is_array($token)) {
            if (isset($token['errcode'])) {
                ...
                $data->access_token = $token['access_token'];
                $this->set_php_file($accessTokenFilePath, json_encode($data));
                return $token;
            }
        } else {
            throw new \Exception('获取微信access_token失败！');
...
class WechatAuth
{
     ...
     * @param AccessToken $token 获取access_token接口
     * @return array access_token信息，包含 token 和有效期
     */
    public function getAccessToken(AccessToken $token)
    {
        return $token->getAccessToken($this->appId, $this->appSecret);
    }

    /**
     * 网页授权获取access_token，用于后续接口访问
     * @return array access_token信息，包含 token
     */
    public function getAccessTokenWithCode($code, $redirectUrl)
    {
        $param = array(
            'appid' => $this->appId,
        ...
        $token = json_decode($token, true);
        //file_put_contents('a.txt',json_encode($token));

        if (is_array($token)) {
            if (isset($token['errcode'])) {
                //$url = U("Home/Index/index", $params, true, true);
     ...
     */
    public function mediaGet($media_id, $accessToken)
    {
        $param = array(
     ...
     * 预览群发
     */
    public function massSend($msgType, $openId, $content)
    {
        $data = array(
            'touser' => $openId,
            'text' => array(
